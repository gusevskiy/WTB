...
def add():
    try:
        ...
        if word in rows:
            ...
            cur.execute(
                f'''INSERT INTO words (word, translate) VALUES ('{w}', '{t}');''')
            con.commit()
...
translate.grid(column=1, row=1, columnspan=3,
               sticky=S, pady=(5, 0), padx=(2, 0))

create = Button(root, text='add', command=add).grid(
    column=0, row=2, sticky=S, pady=(5, 0), padx=(2, 0))
delete = Button(root, text='delete', command=delite_word).grid(
    column=1, row=2, sticky=S, pady=(5, 0), padx=(2, 0))
printW = Button(root, text='print', command=print_word).grid(
    column=2, row=2, columnspan=3, sticky=S, pady=(5, 0), padx=(2, 0))


translate_word = Listbox(root, height=30, width=30)

translate_word.grid(column=0, row=4, columnspan=4,
                    sticky=S, pady=(5, 0), padx=(2, 0))
